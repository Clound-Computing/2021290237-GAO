    ...
    for example_index, example in enumerate(examples):
        ...
        if labels_to_use != None:
            if labels_to_use[example_index] == None:
                ...
                if labels_to_use[example_index] != None:
                    label_selected = label_map_reverse[labels_to_use[example_index]]
                    answer_str = example.choices[label_selected]
                else:
                    answer_str = 'one of the choices'
        else:
        ...
        if explanations_only:
            sent1 = ""
            sent2 = ""
        input_str_raw = f"You are given two sentences: 1: {sent1} 2: {sent2}"
        input_str_formatted = template_fn(input_str_raw + ' Is the first sentence or the second sentence commonsense? Answer only with: first or second:', is_input=True)

        if condition_on_explanations and not multi_explanation:
            input_str_formatted = template_fn(input_str_raw + f" Is the first sentence or the second sentence commonsense? My commonsense tells me {explanation_str} Answer only with first or second: ", is_input=True)
        elif condition_on_explanations and multi_explanation:
            # import pdb
            # pdb.set_trace()
            # TODO check we're getting all 3 explanations in correct form
            explanations = ""
            for i, exp in enumerate(example.explanation_list):
                explanations += f"Explanation for label {example.choices[i]}: {exp};"
            input_str_formatted = template_fn(input_str_raw + f" Is the first sentence or the second sentence commonsense? Possible explanations are: {explanations} Answer only with first or second:", is_input=True)
        ...
        if do_explain and multi_explanation:
            # Explain (in a compact, non-repetitve, and succinct way) why the {answer_str} sentence is common sense and not the other:
            explanation_input = template_fn(input_str_raw + f'; Give a reason that explains why the {answer_str} sentence does not make sense: ', is_input=True)
        else:
            explanation_input = template_fn(input_str_raw + '; Give a reason that explains why one of the sentences does not make sense: ', is_input=True)

        data_point = {'input_str': input_str_formatted if len(task_input_str_list) < 1 else task_input_str_list,
